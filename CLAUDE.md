...
**Digest pipeline:** Collector → Ranker (LLM top-N selection) → Scraper → Analyzer (LLM Chinese analysis) → Notifier

**Hot pipeline (`main_hot.py`):** Collector (with comment counts) → sort by comment count → Scraper → Analyzer → Notifier. Skips LLM ranking entirely.
